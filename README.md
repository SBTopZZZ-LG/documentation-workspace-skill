...
This repository is the **source** of the `documentation-workspace` skill. The artifact an agent runtime actually consumes is [`skill/SKILL.md`](skill/SKILL.md) — the file the skill loader reads when a user triggers the skill. Everything else in this repo supports the skill: templates it instantiates, scripts it runs, references it may load.
...
The skill runs **once per workspace**. After that, the workspace's own `AGENTS.md` takes over and the skill is never re-invoked. This is the whole point: pre-decide the rules once, then never burn tokens re-deriving them.
...
|---|---|---|
| 1. **The skill** | this repo (`skill/`) | Generic, model-agnostic trigger + bootstrap workflow. The agent reads this once, when the user asks to bootstrap. |
...
├── AGENTS.md                            ← agent-facing repo rules (this repo)
├── .gitignore
└── skill/                               ← THE SKILL — what gets loaded by an agent runtime
    ├── SKILL.md                         ← frontmatter (description) + workflow
    ├── assets/
...
After install, the skill triggers on phrases like *"set up a docs workspace"*, *"scaffold a docs folder"*, *"organize my notes into folders"*, etc. (see `skill/SKILL.md` for the full description).
...
node skill/assets/skill-scripts/bootstrap.js \
...
| Changing… | Edit |
|---|---|
| The skill's behavior or trigger description | `skill/SKILL.md` |
| The per-workspace rules | `skill/assets/templates/AGENTS.md.template` |
| Workspace-shipped scripts | `skill/assets/workspace-scripts/*.js` |
| The bootstrap orchestrator | `skill/assets/skill-scripts/bootstrap.js` |
| Optional reference docs | `skill/references/*.md` |
...
If you want any of these changed, edit `skill/assets/templates/AGENTS.md.template` — **not** `SKILL.md` and **not** a one-off example.
...
- `skill/SKILL.md` — the skill the agent reads when triggered.
- `skill/assets/templates/AGENTS.md.template` — the heart of the system; most iteration will happen here.
- `skill/references/asset-management.md` — content-addressed asset workflow.
- `skill/references/document-promotion.md` — promoting a single-file doc to a folder.
